...
export function UserPostCard({ id, userId, userName, caption, images, likes: initialLikes, createdAt, onPostDeleted }: UserPostCardProps) {
  ...
  return (
    <div className="bg-card border border-border rounded-lg shadow-sm mb-4">
      ...
      {images.length > 0 && (
        <div className="border-b border-border bg-black overflow-hidden">
          {images.length === 1 ? (
            ...
            <div className="flex gap-1">
              {images.map((image, index) => (
                <img
                  ...
                  className="flex-1 h-64 object-cover cursor-pointer hover:opacity-90 transition-opacity"
                />
              ))}
            </div>
          ) : images.length === 3 ? (
            ...
            <div className="flex gap-1">
              <img
                ...
                className="flex-1 h-80 object-cover cursor-pointer hover:opacity-90 transition-opacity"
              />
            ...
            /* 5+ images - large on left, grid on right with +N overlay */
            <div className="flex gap-1">
              <img
                src={images[0]}
                alt="Post image 1"
            ...
            <div className="flex-1">
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-slate-100 border border-border rounded-full px-4 py-2 text-sm text-foreground placeholder-muted-foreground outline-none focus:ring-2 focus:ring-primary transition-all"
                />
                <button
                  type="submit"
                ...
                {replyingTo === comment.id && (
                  <form onSubmit={(e) => handleAddReply(comment.id, e)} className="ml-11 flex gap-2">
                    <input
                      ...
                      placeholder="Reply..."
                      className="flex-1 bg-slate-100 border border-border rounded-full px-3 py-1 text-sm text-foreground placeholder-muted-foreground outline-none focus:ring-2 focus:ring-primary transition-all"
                      autoFocus
                    />
                    <button
                      type="submit"
